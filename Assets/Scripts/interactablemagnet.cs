...
public class interactablemagnet : MonoBehaviour {
	...
	void OnCollisionEnter (Collision target) {
		if (runOnce == false) {
			if (target.gameObject.tag.Equals ("chainlink") == true) {
				Debug.Log ("MAG");
				mag = true;
				selected = false;
				selectedObject = null;
				...
				target.gameObject.GetComponent<Rigidbody> ().AddForce (transform.right * -40);
				runOnce = true;
				fanObj.GetComponentInChildren<Animation>().Play("Take 001");
				apObj.GetComponent<Interactable>().ApplyForce();
			}
		}
	}
